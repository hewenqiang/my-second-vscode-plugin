...
class AIAssistantPanel {
    static createOrShow(extensionUri, initialText) {
        ...
        // 否则创建新面板
        const panel = vscode.window.createWebviewPanel(AIAssistantPanel.viewType, 'AI助手', column || vscode.ViewColumn.One, {
            enableScripts: true,
            localResourceRoots: [
    ...
    constructor(panel, extensionUri, initialText) {
        this._disposables = [];
        this._contextItems = [];
        this._panel = panel;
...
<body>
    ...
    async _fetchPublicContext(source) {
        // 支持若干免费 API，便于测试
    ...
    async _handleAskQuestion(question) {
        ...
        try {
            ...
            const res = await (0, undici_1.fetch)(endpoint, {
                method: 'POST',
                headers: {
    ...
    async _handleFetchContext(source) {
        try {
...
AIAssistantPanel.viewType = 'aiAssistantPanel';
